...
class WindowsOCREngine:
    ...
    async def _recognize(self, np_image: Any) -> Any:
        ...
        import winrt.windows.storage.streams as wstr
        from PIL import Image

        # Dil etiketini dene, bulamazsa sistem dilini kullan
        lang = wglob.Language(self.lang_tag)
        engine = wocr.OcrEngine.try_create_from_language(lang)
        if engine is None:
            # "en" dil etiketi çalışmıyor olabilir — sistem profil dilini dene
            engine = wocr.OcrEngine.try_create_from_user_profile_languages()
        if engine is None:
            _log("[WinOCR] Engine oluşturulamadı — dil yüklü değil?")
            return None

        # numpy BGRA → RGB PIL Image
        img_arr = np_image
        if img_arr.ndim == 2:
        ...
        data = buf.getvalue()   # bytes — winrt write_bytes bytes bekliyor

        stream = wstr.InMemoryRandomAccessStream()
        writer = wstr.DataWriter(stream.get_output_stream_at(0))
    ...
    def read(self, np_image: Any) -> List[Tuple[Any, str, float]]:
        ...
        try:
            result = asyncio.run(self._recognize(np_image))
            if not result:
                return lines
            for line in result.lines:  # type: ignore
                words = line.words
                if not words:
                    continue
                x0 = min(int(w.bounding_rect.x) for w in words)
                y0 = min(int(w.bounding_rect.y) for w in words)
                x1 = max(int(w.bounding_rect.x + w.bounding_rect.width)  for w in words)
                y1 = max(int(w.bounding_rect.y + w.bounding_rect.height) for w in words)
                bbox = [[x0, y0], [x1, y0], [x1, y1], [x0, y1]]
                lines.append((bbox, line.text, 0.9))
...
class HybridOCREngine:
    ...
    def _fallback_read(self, image: Any) -> List[Tuple[Any, str, float]]:
        ...
        if sc >= self.QUALITY_THRESHOLD:
            return win_result
        _log(f"[HybridOCR] Skor düşük ({sc}<{self.QUALITY_THRESHOLD}) → EasyOCR")
        ...
        with ThreadPoolExecutor(max_workers=2) as ex:
            ...
            for fut in as_completed(futs):
                ...
                try:
                    ...
                    _log(f"[HybridOCR parallel] {key} hata: {exc}")
                    results[key] = []
...
class HardwareDetector:
    ...
    def detect() -> Dict[str, Any]:
        ...
        _log(f"=== Donanım Taraması Başlıyor (Python {sys.version}) ===")
...
class OCRProcessor:
    ...
    def process(image: Any) -> Tuple[Any, int]:
        ...
        # Adaptif modda pixel_count'u proxy olarak döndür (OCR kararı için)
        return adap, adap_px
...
class TaskEngine:
    ...
    def _loop(self) -> None:
        ...
        if mss is None or np is None or cv2 is None:
            _log("[_loop KRITIK] mss / numpy / cv2 kurulu değil!")
            self.q.put({"a": "ocr", "t": "⚠ mss/cv2/numpy eksik — tam_kurulum.bat çalıştırın"})
            return
        if _active_ocr_engine is None:
            _log("[_loop KRITIK] OCR motoru henüz yüklenmedi, 3sn bekleniyor...")
            time.sleep(3)
            if _active_ocr_engine is None:
                _log("[_loop KRITIK] OCR motoru hâlâ None — döngü başlamıyor!")
...
# ── Dil Yönü (Altyazı Paneli İçi) ───────────────────────────────────
    ...
    def start(self) -> None:
        ...
        self.app._refresh_glow()  # Motor başladı, diğerlerinin ışığını söndür
    ...
    def stop(self) -> None:
        ...
        self.app._refresh_glow()  # Motor durdu, hazır olanların ışığını (sarı) geri yak
...
# ── Ana Alan ─────────────────────────────────────────────────────────────
    ...
    def _refresh_engine_cards(self) -> None:
        ...
        # ec_row'u bulmak için daha sağlam bir yol
        # self._prog_bar.master, eng_card'dır. eng_card'ın çocukları arasında ec_row bulunur.
        # ec_row, eng_card'ın 3. çocuğu (0: label, 1: ec_row, 2: prog_bar, 3: prog_lbl)
        # Ancak, _prog_bar ve _prog_lbl'nin pack() ile eklenmesi, grid() ile eklenen ec_row'un
        # çocuk indeksini değiştirebilir. En iyisi ec_row'u doğrudan saklamak.
        # Geçici çözüm: eng_card'ın çocuklarını döngüye alıp ctk.CTkFrame olanı bulmak.
        ec_row = None
        for child in self._prog_bar.master.winfo_children():
            if isinstance(child, ctk.CTkFrame) and child.winfo_name() != "!ctkframe": # !ctkframe label'ı atla
                if child.winfo_children(): # İçinde başka widget'lar olan frame'i bul
                    ec_row = child
                    break
        if not ec_row:
            _log("Hata: ec_row bulunamadı.")
            return
    ...
    def _install_thread(self, task_key: str) -> None:
        ...
        try:
            ...
            if cancelled or self._install_stop.is_set():
                _log(f"[Kurulum] İptal/geri alma: {self._installed_pkgs}")
                ...
                if need_opencv:
                    ...
                    _log("[Geri alma] opencv-python yeniden kuruluyor…")
    ...
    def _finish_install(self, cancelled: bool = False, engine_key: str = "") -> None:
        ...
        self._install_stop.clear()
        self._installed_pkgs.clear()
        # Motor kartı butonlarını normal moda döndür
        for key, frame in list(self._eng_frames.items()):
            parent = frame.master
            col = list(self._eng_frames.keys()).index(key)
            frame.destroy()
            new_f = self._make_engine_card(parent, key, self.ENGINE_META[key])
            new_f.grid(row=0, column=col, padx=5, sticky="nsew")
            self._eng_frames[key] = new_f
        self._refresh_glow()
    ...
    def _apply_style(self, *_: Any) -> None:
        ...
        # SADECE panel_sub KALDI!
        if self.panel_sub.engine.overlay:
            if not any(self.panel_sub.engine.overlay._lines):
                # Ekranda yazı yoksa anında görmek için önizleme at
                if not any(panel.engine.overlay._lines):
                    panel.engine.overlay.push_line("Ayar Önizlemesi...")
                panel.engine.overlay.set_style(
    ...
    def _init_ocr(self) -> None:
        ...
        def _load() -> None:
            import traceback
            global _active_ocr_engine
            _log(f"--- Uygulama Başlatıldı: Python {sys.version} ---")
            try:
                ...
                if sel_engine == "hybrid":
                    ...
                    if _active_ocr_engine is None:
                        _log("KRITIK: OCR motoru None döndü — hiçbir motor yüklenemedi!")
   ...
   # ── Rastgele ve Dinamik Açılış Ekranı (Splash Screen) ─────────────────────
    ...
    def _close_splash(self) -> None:
        ...
        # Ses ve logo işi bitti, ana programı ihtişamla ekrana getir
        self.root.deiconify()  
